...
interface PDFViewerProps {
    file: FileInfo;
    onTextSelected: (text: string, position: { x: number; y: number; screenX: number; screenY: number }) => void;
}

export default function PDFViewer({ file, onTextSelected }: PDFViewerProps) {
    ...
    const loadPDF = async () => {
        ...
        try {
            let pdfSource: any;
            if (file.localFile) {
                const arrayBuffer = await (file.localFile as File).arrayBuffer();
                pdfSource = { data: arrayBuffer };
            } else {
                pdfSource = { url: file.fileUrl || file.url || "" };
            }
